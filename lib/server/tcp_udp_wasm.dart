...
class TcpUdpWasm {
  ...
  static Future<void> _init() async {
    ...
    _module = js_util.getProperty(globalThis, 'TcpUdpModule');
    if (_module != null) return;

    // Try to create it manually if factory function exists
    final createFn = js_util.getProperty(globalThis, 'createTcpUdpModule');
    if (createFn != null) {
      final jsResult = js_util.callMethod(globalThis, 'createTcpUdpModule', []);
      try {
        _module = await js_util.promiseToFuture(jsResult);
      } catch (_) {
        _module = jsResult;
      }
      js_util.setProperty(globalThis, 'TcpUdpModule', _module);
      return;
    }
    ...
    throw Exception('WASM module not found. Make sure tcp_udp.js is loaded.');
  }
  ...
  static Future<String> analyze(String headerHex) async {
    ...
    final jsWrapper = js_util.callMethod(
      _module,
      'cwrap',
      ['analyze_transport_header', 'number', ['string']],
    );
